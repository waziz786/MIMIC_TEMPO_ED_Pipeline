-- ECG Features Window 6 (W6): First ECG within 6 hours of ED arrival
-- 
-- Extracts machine-derived ECG measurements for the first ECG recorded
-- within 6 hours of ED arrival. Features are derived from:
...
DROP TABLE IF EXISTS {ecg_features_w6};

CREATE TABLE {ecg_features_w6} AS
WITH candidates AS (
  -- Find all ECGs within 6 hours of ED arrival
  SELECT
    b.stay_id,
    b.subject_id,
    r.study_id,
  ...
  WHERE r.ecg_time IS NOT NULL
    AND r.ecg_time BETWEEN b.ed_intime AND LEAST(b.ed_outtime, b.ed_intime + INTERVAL '6 hours')
),
first_ecg AS (
...
SELECT
  b.stay_id,
  f.study_id AS ecg_study_id_w6,
  f.ecg_time AS ecg_time_w6,
  f.hours_from_ed AS ecg_hours_from_ed_w6,
  ...
  m.rr_interval AS ecg_rr_interval_w6,
  ...
  m.p_onset AS ecg_p_onset_w6,
  m.p_end AS ecg_p_end_w6,
  m.qrs_onset AS ecg_qrs_onset_w6,
  m.qrs_end AS ecg_qrs_end_w6,
  m.t_end AS ecg_t_end_w6,

  -- Axis measurements (degrees)
  m.p_axis AS ecg_p_axis_w6,
  m.qrs_axis AS ecg_qrs_axis_w6,
  m.t_axis AS ecg_t_axis_w6,

  -- Missing indicator
  CASE WHEN f.study_id IS NULL THEN 1 ELSE 0 END AS missing_ecg_w6
...
CREATE INDEX IF NOT EXISTS idx_{ecg_features_w6}_stay_id ON {ecg_features_w6}(stay_id);
